...
<!--      part2-->
      ...
      <div v-show="IsShow">
        ...
        <div v-for="(order,index) in orderList" :key="order.id">
          ...
          <div >
            <button type="button" class="btn btn-default" style="margin-left: 45%;border: solid #cbc6c6 1px;border-radius: 20px;width:25%;color: #cbc6c6" @click="cancelOrder(order)">取消订单</button>
            <button type="button" class="btn btn-default" style="border: solid #f63333 1px;border-radius: 20px;color:#f63333;width:25%" @click="paymentTap(order)">去付款</button>
          </div>
